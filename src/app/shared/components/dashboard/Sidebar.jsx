...
import { menuKeys, rolePermissions, userRoles, isAdminRole } from '../../../../types/roles';

/**
 * Sidebar Component with RBAC
 * 
 * Dynamically displays menu items based on user role and current area (vendor/admin).
 * Uses the access control matrix defined in types/roles.ts.
 */
const Sidebar = () => {
    const { user } = useAuth();
    ...
    const userRole = user?.role || userRoles.VENDOR;
    ...
    const allMenuItems = [
        ...
        {
            icon: <FileText size={20} />,
            label: 'Vérification FNE',
            path: `${basePath}/fne-verification`,
            key: menuKeys.VERIFICATION_FNE
        },
    ...
    const menuItems = allMenuItems.filter((item, index, array) => {
        if (item.type === 'header') {
            ...
            for (let i = index + 1; i < array.length; i++) {
                ...
                if (allowedKeys.includes(array[i].key)) {
