...
const AddUserModal = ({ isOpen, onClose, onSave, user = null }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            if (user) {
                setFormData({
                    ...user,
                    password: user.password || '' // Existing password if any
                });
            } else {
                setFormData({
                    name: '',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            showSuccess(user ? 'Utilisateur mis à jour avec succès' : 'Utilisateur créé avec succès');
            onClose();
        } catch (error) {
            console.error(error);
                ...
                <div style={{
                    ...
                    <div>
                        ...
                        <p style={{ margin: 0, fontSize: '0.875rem', color: 'rgba(255, 255, 255, 0.8)' }}>
                            {user ? 'Modifiez les informations ci-dessous.' : 'Remplissez les informations ci-dessous.'}
                        </p>
                    </div>
                    <button
                        onClick={onClose}
